...
class _ExamDetailScreenState extends ConsumerState<ExamDetailScreen> with TickerProviderStateMixin {
  ...
  void _initializeTimer() {
    ...
    final endTimeStr = exam['endTime'] as String?;
    if (endTimeStr != null) {
  ...
  void _triggerScanForQuestion(Map<String, dynamic> question) {
  ...
  void _finalizeSubmission(String examToken) async {
    ...
    try {
      ...
      final response = await paperRepo.finalizeSubmission(examToken);
      ...
      if (mounted) {
        ResponseDialog.show(
          ...
          message: 'An error occurred while locking your submission: ${e.toString()}',
  ...
  Widget build(BuildContext context) {
    ...
    final String examId = exam['_id'] ?? '';
    final subjectName = exam['subjectName'] ?? 'Subject Name';
    final subjectCode = exam['subjectCode'] ?? 'CODE';
    final examType = exam['examType'] ?? 'Examination';
    final semester = exam['semester']?.toString() ?? 'N/A';
    final course = exam['course'] ?? 'N/A';
    final maxMarks = exam['maxMarks']?.toString() ?? '100';
    ...
    Future<bool> showExitConfirmation(BuildContext context) async {
      final shouldExit = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          ...
          title: const Text('Exit Exam Portal?', style: TextStyle(fontWeight: FontWeight.bold, color: AppTheme.textPrimary)),
          content: const Text(
            'Your active script upload session is currently running. Exiting now will not finalize or secure your submission. Are you sure you want to go back?',
              ...
              child: TabBarView(
                ...
                children: [
                  ...
                  SingleChildScrollView(
                    ...
                    child: Column(
                      children: [
                        _buildHeroCard(subjectName, subjectCode, examType, semester, course, maxMarks),
  ...
  Widget _buildQuestionsList(Map<String, dynamic> exam, AsyncValue<Map<String, String>> submissionsState) {
    final questionPaper = exam['questionPaper'] as Map<String, dynamic>?;
    final examToken = exam['token'] as String;

    if (questionPaper == null || questionPaper['sections'] == null) {
      return const Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
    ...
    final sections = questionPaper['sections'] as List<dynamic>;
            ...
            error: (err, stack) => Center(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 12),
                  ElevatedButton(
                    onPressed: () => ref.refresh(examSubmissionsProvider(exam['_id'])),
                    child: const Text('Retry Connection'),
                  ),
                ],
              ),
            ),
            data: (uploadsMap) => ListView.builder(
              padding: const EdgeInsets.all(20),
              itemCount: sections.length,
              itemBuilder: (context, sectionIndex) {
                final questions = sections[sectionIndex] as List<dynamic>;
                final sectionLabel = String.fromCharCode(65 + sectionIndex); // A, B, C...
  ...
  Widget _buildQuestionItem(Map<String, dynamic> q, Map<String, String> uploadsMap) {
    final questionId = q['questionId'] as String;
    final text = q['text'] as String;
    final marks = q['marks']?.toString() ?? '5';
    final hasUploaded = uploadsMap.containsKey(questionId);
    
    final children = q['children'] as List<dynamic>?;
      ...
      child: Column(
        ...
        children: [
          ...
          if (children != null && children.isNotEmpty) ...[
            ...
            const SizedBox(height: 12),
            ...children.map((subQRaw) {
              final subQ = subQRaw as Map<String, dynamic>;
              final subId = subQ['questionId'] as String;
              final subText = subQ['text'] as String;
              final subMarks = subQ['marks']?.toString() ?? '2';
  ...
  Widget _buildHeroCard(String title, String code, String type, String sem, String course, String marks) {
    return Container(
      padding: const EdgeInsets.all(24),
      decoration: BoxDecoration(
        color: AppTheme.surfaceColor,
