...
export const getExamByIdByToken = async (req, res) => {
  try {
    ...
    let examToken = token;
    let scannedTimestamp = null;
    let scannedSignature = null;

    if (token.includes(":")) {
      const parts = token.split(":");
      examToken = parts[0];
      scannedTimestamp = parseInt(parts[1], 10);
      scannedSignature = parts[2];
    }

    // 2. Find exam using token
    const exam = await Exam.findOne({ token: examToken })
    ...
    // 2.5. Verify Dynamic HMAC Time Signature
    if (scannedTimestamp && scannedSignature) {
      const crypto = await import("crypto");
      const secret = process.env.HMAC_SECRET;
      if (!secret) throw new Error("HMAC_SECRET environment variable is not configured.");
      const expected = crypto.createHmac("sha256", secret)
        .update(`${examToken}:${scannedTimestamp}`)
        .digest("hex");

      if (scannedSignature !== expected) {
        return res.status(403).json({
          success: false,
          message: "Access Denied: Invalid or forged exam hall QR code signature.",
        });
      }

      // Normalize scanTimeMs handling 13-digit ms, 10-digit sec, or minute indexes
      let scanTimeMs = scannedTimestamp;
      if (scannedTimestamp < 1e7) {
        scanTimeMs = scannedTimestamp * 60000;
      } else if (scannedTimestamp < 1e11) {
        scanTimeMs = scannedTimestamp * 1000;
      }

      const examStart = exam.startTime.getTime();
      const examEnd = exam.endTime.getTime();

      if (scanTimeMs < examStart - 10 * 60000 || scanTimeMs > examEnd + 10 * 60000) {
        return res.status(403).json({
          success: false,
          message: "Access Denied: The scanned exam QR code has expired or is outside the scheduled exam window.",
        });
      }
    }
    ...
    if (exam.collegeId.toString() !== student.collegeId.toString()) {
      return res.status(403).json({
        success: false,
...
export const startUploadSession = async (req, res) => {
  try {
    ...
    if (exam.collegeId.toString() !== student.collegeId.toString()) {
      return res.status(403).json({ success: false, message: "Access denied. College ID mismatch." });
    }
    ...
    // 2. Guard: Block session creation if student has already finalized this exam
    const existingAnswers = await Answers.findOne({ uploaded_student: studentId, for_exam: exam._id });
    if (existingAnswers && existingAnswers.isSubmitted) {
      return res.status(403).json({ success: false, isAlreadySubmitted: true, message: "You have already submitted this exam. Re-attempts are strictly prohibited." });
    }

    // 3. Check if the student already has an active session for this exam
...
export const getStudentTimetableAndExams = async (req, res) => {
  try {
    ...
    .select('-token -qrCode') // Exclude secret token and QR code
    .populate('questionPaper')
    .populate('assignedFaculty', 'name email')
    .populate('courseId')
    .populate('semesterId', 'semesterNumber semesterName')
    ...
    const submittedExamIds = new Set(submissions.filter(s => s.isSubmitted).map(s => s.for_exam.toString()));
    const resultExamIds = new Set(results.map(r => r.exam.toString()));

    const sanitizeQuestionPaperStructure = (paper) => {
    ...
    const examsWithSubmission = exams.map(exam => {
      ...
      // Sanitize question paper: send section & questionId metadata beforehand, hide secret text/images!
      if (examObj.questionPaper) {
        examObj.questionPaper = sanitizeQuestionPaperStructure(examObj.questionPaper);
      }
